...
const perfectThrottle = (callback, timer) => {
    let lastFunc;
    let lastRan;
    return function () {
        let context = this;
...
const parseFunction = (obj) => {
    ...
    if (Array.isArray(value)) {
        let result;
        switch (key) {
            case 'AND': result = value.reduce((accumulater, value) => accumulater & value, value[0]);
            case 'OR': result = value.reduce((accumulater, value) => accumulater | value, value[0]);
            case 'NOT': result = !value[0];
        }
        return result;
    }
